...
function getCandidates($, base) {
  ...
  $('*', 'body').each(function(index, element) {
    var node = $(this);
    if (!node || node.length == 0) return; // Avoid Illegal Access error
    ...
    // score paragraph direct.
    if (nodeType == "p") {
      scoreNode(node, candidates);
    }
...
// save score as node's data.
function scoreCandicate(parent, contentScore, candidates){
  var score;
  if (typeof parent.data('readabilityScore') == "undefined") {
    score = initializeNode(parent);
    candidates.push(parent);
  }else{
    score = parent.data('readabilityScore') || 0;
  }
  parent.data('readabilityScore', score + contentScore)
}
// nodes need score
function scoreNode(node, candidates){
  ...
  if(parent && parent.length > 0){
    scoreCandicate(parent, contentScore, candidates);
    var grandParent = parent.parent();
    if(grandParent && grandParent.length > 0){
      scoreCandicate(parent, contentScore/2, candidates);
    }
  }
}
/**
