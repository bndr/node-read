...
var trashNodes = 'meta,iframe,noscript,style,aside,object,form';
...
function getArticle(candidates, $) {
  ...
  candidates.forEach(function(elem) {
    ...
    elem.data('readabilityScore', Math.min(siblings, 4) * score * (1 - linkDensity));
...
function getCandidates($, base) {
  ...
  // Remove Irrelevat stuff
  $('body').contents().filter(function(i, node) {
    if (node.type === "comment" || node.type == "script") return true;
    if (trashNodes.indexOf(node.name) != -1) return true;
  }).remove();
